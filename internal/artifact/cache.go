...
const DefaultCacheBytes = int64(8) << 30

// CacheStore is a read-through cache in front of a remote BlobStore.
//
...
func NewCacheStore(remote BlobStore, options CacheOptions) (*CacheStore, error) {
	if remote == nil {
...
func (store *CacheStore) Put(
	...
	if err != nil {
		return BlobInfo{}, fmt.Errorf("artifact: reopen staged object: %w", err)
	}
	info, putErr := store.remote.Put(ctx, storageKey, file, maxBytes)
	closeErr := file.Close()
	if putErr != nil {
		return BlobInfo{}, putErr
	}
	if closeErr != nil {
...
func (store *CacheStore) Delete(ctx context.Context, storageKey string) error {
	if err := ValidateStorageKey(storageKey); err != nil {
		return err
	}
...
func (store *CacheStore) Check(ctx context.Context) error {
...
func (store *CacheStore) hit(storageKey string) (io.ReadCloser, BlobInfo, bool) {
	...
	return file, BlobInfo{storageKey: storageKey, sizeBytes: info.Size()}, true
}

func (store *CacheStore) fill(ctx context.Context, storageKey string) error {
...
func (store *CacheStore) stage(
	...
	if err := os.MkdirAll(store.stagingDir(), 0o750); err != nil {
		return "", CopyResult{}, fmt.Errorf("artifact: create cache staging: %w", err)
	}
	staged, err := os.CreateTemp(store.stagingDir(), "fill-*")
	if err != nil {
		return "", CopyResult{}, fmt.Errorf("artifact: stage cache object: %w", err)
	}
	path := staged.Name()
	result, copyErr := CopyAndHashContext(ctx, staged, source, maxBytes)
	if copyErr == nil {
		copyErr = staged.Sync()
	}
...
func (store *CacheStore) publish(storageKey, staged string, size int64) {
...
func (store *CacheStore) touch(storageKey string, size int64) {
	store.mu.Lock()
...
func (store *CacheStore) evict(storageKey string) {
...
func (store *CacheStore) stagingDir() string {
	return filepath.Join(store.root, ".staging")
}

func (store *CacheStore) path(storageKey string) (string, error) {
	if err := ValidateStorageKey(storageKey); err != nil {
		return "", err
	}
	...
	if err != nil || relative == ".." ||
		...
		return "", fmt.Errorf("%w: key escapes cache", ErrInvalidStorageKey)
	}
	return target, nil
}

var _ BlobStore = (*CacheStore)(nil)
