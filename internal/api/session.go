...
const SessionCookieName = "neurun_session"

// sessionPrincipal maps a session onto the same Principal the API-key path
// produces, so scope enforcement has exactly one implementation.
func sessionPrincipal(session session.Session) auth.Principal {
...
func (server *Server) login(ctx *gin.Context) {
	...
	session, token, err := server.sessions.Login(
	...
	ctx.JSON(http.StatusOK, gin.H{"session": dto.NewSessionResponse(session)})
}
