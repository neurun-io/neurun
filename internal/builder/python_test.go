...
func TestPythonBuilderRejectsMissingEntrypoint(t *testing.T) {
	...
	_, err = builder.Build(context.Background(), deployment.BuildRequest{Runtime: deployment.RuntimePython, EntryPoint: "main.py:handler", SourceArchivePath: source, WorkDirectory: filepath.Join(root, "work")})
	if err == nil {
...
func buildFixture(t *testing.T, python string) string {
	...
	result, err := builder.Build(context.Background(), deployment.BuildRequest{Runtime: deployment.RuntimePython, EntryPoint: "main.py:handler", SourceArchivePath: source, WorkDirectory: work})
	if err != nil {
		t.Fatal(err)
	}
