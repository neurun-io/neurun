...
A machine that holds one app resident and exposes an endpoint.

**Not built.** There is no `/v1/servers`, no server in the dashboard, and no
...
away. A server inverts it: it pins one app to one ready build, keeps it resident,
...
| Metered | compute consumed while running | time the server is up |
...
double-counting the same second. A server is therefore its own meter and its own
invoice line. Nothing about execution pricing changes when servers ship.
...
- a lifecycle the server owns — starting, ready, draining, stopped — carrying
  the reason a server left `ready`;
- the exposed endpoint and its authentication, scoped to the server rather than
  to a project key;
...
The dashboard's `/servers` route names this same list rather than rendering a
server it cannot start, stop or bill.
