...
func (builder *PythonBuilder) Build(ctx context.Context, request deployment.BuildRequest) (deployment.BuildResult, error) {
	if request.Runtime != deployment.RuntimePython {
		return deployment.BuildResult{}, fmt.Errorf("builder: unsupported runtime %q", request.Runtime)
	}
	if ctx == nil {
		ctx = context.Background()
	}
	if strings.TrimSpace(request.SourceArchivePath) == "" || strings.TrimSpace(request.WorkDirectory) == "" {
		return deployment.BuildResult{}, errors.New("builder: source archive and work directory are required")
	}
	...
	if _, err := artifact.ExtractZIPFile(request.SourceArchivePath, sourceDir, builder.limits); err != nil {
		return deployment.BuildResult{}, fmt.Errorf("builder: extract source: %w", err)
	}
	...
	if output, err := compile.CombinedOutput(); err != nil {
		return deployment.BuildResult{}, commandError("compile Python source", output, err)
	}
	codePath := filepath.Join(request.WorkDirectory, "code-layer.zip")
	if err := zipDirectory(sourceDir, codePath); err != nil {
		return deployment.BuildResult{}, fmt.Errorf("builder: package code layer: %w", err)
	}
	result := deployment.BuildResult{Artifacts: []deployment.BuiltArtifact{{Kind: deployment.ArtifactCodeLayer, Name: "code-layer.zip", MediaType: "application/zip", Path: codePath}}}
	...
	if err != nil {
		return deployment.BuildResult{}, fmt.Errorf("builder: inspect requirements.txt: %w", err)
	}
	if !info.Mode().IsRegular() {
		return deployment.BuildResult{}, errors.New("builder: requirements.txt must be a regular file")
	}
	contents, err := os.ReadFile(requirements)
	if err != nil {
		return deployment.BuildResult{}, fmt.Errorf("builder: read requirements.txt: %w", err)
	}
	...
	if err := os.Mkdir(installDir, 0o700); err != nil {
		return deployment.BuildResult{}, fmt.Errorf("builder: create install layer: %w", err)
	}
	...
	if output, err := pip.CombinedOutput(); err != nil {
		return deployment.BuildResult{}, commandError("install Python requirements", output, err)
	}
	installPath := filepath.Join(request.WorkDirectory, "install-layer.zip")
	if err := zipDirectory(installDir, installPath); err != nil {
		return deployment.BuildResult{}, fmt.Errorf("builder: package install layer: %w", err)
	}
	result.Artifacts = append(result.Artifacts, deployment.BuiltArtifact{Kind: deployment.ArtifactInstallLayer, Name: "install-layer.zip", MediaType: "application/zip", Path: installPath})
	return result, nil
}
...
var _ deployment.Builder = (*PythonBuilder)(nil)
