...
// countingStore is a Memory that records how often it was read, which is
...
func newCache(t *testing.T, remote Repository, maxBytes int64) *Cache {
	t.Helper()
	cache, err := NewCache(remote, CacheOptions{
...
func read(t *testing.T, store Repository, key string) string {
	t.Helper()
...
func TestCacheServesRepeatedReadsWithoutTouchingTheRemote(t *testing.T) {
	...
	if _, err := cache.Put(
		context.Background(), objectKey, strings.NewReader("payload"), 1_048_576,
	); err != nil {
		t.Fatalf("put: %v", err)
	}
...
func TestCacheEvictsLeastRecentlyUsedToStayInBudget(t *testing.T) {
	...
	for _, key := range keys[:2] {
		if _, err := cache.Put(
			context.Background(), key, bytes.NewReader(make([]byte, 10)), 1_048_576,
		); err != nil {
	...
	read(t, cache, keys[0])
	if _, err := cache.Put(
		context.Background(), keys[2], bytes.NewReader(make([]byte, 10)), 1_048_576,
	); err != nil {
